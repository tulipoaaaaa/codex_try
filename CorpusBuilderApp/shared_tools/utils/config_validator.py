...
try:
    ...
    import balancer_config, domain_config
...
def main(json_output_path=None):
    ...
    for domain, ref_cfg in REFERENCE_DOMAINS.items():
        if domain not in EXTRACTOR_DOMAINS:
            logger.warning(f"[ERROR] Domain '{domain}' missing in domain_config.py!")
            ...
            if alloc != target:
                logger.warning(f"[WARNING] Domain '{domain}' allocation mismatch: domain_config={alloc}, balancer_config={target}")
                ...
                logger.info(f"[OK] Domain '{domain}' matches.")
    # 2. Check for extra domains in domain_config.py
    for domain in EXTRACTOR_DOMAINS:
        if domain not in REFERENCE_DOMAINS:
            logger.info(f"[INFO] Domain '{domain}' is extra in domain_config.py (not used by balancer)")
    ...
    if abs(ref_sum - 1.0) > 1e-6:
        logger.warning(f"[ERROR] balancer_config.py weights sum to {ref_sum} (should be 1.0)")
    else:
        logger.info(f"[OK] balancer_config.py weights sum to 1.0")
    if abs(ext_sum - 1.0) > 1e-6:
        logger.warning(f"[ERROR] domain_config.py allocations sum to {ext_sum} (should be 1.0)")
    else:
        logger.info(f"[OK] domain_config.py allocations sum to 1.0")
    ...
    for domain in set(list(REFERENCE_DOMAINS.keys()) + list(EXTRACTOR_DOMAINS.keys())):
        if not is_valid_identifier(domain):
            logger.warning(f"[ERROR] Invalid domain name: '{domain}' (not a valid Python identifier)")
    ...
    if ref_duplicates:
        logger.warning(f"[ERROR] Duplicate domain names in balancer_config.py: {ref_duplicates}")
        REPORT["duplicate_domains"] += list(ref_duplicates)
    if ext_duplicates:
        logger.warning(f"[ERROR] Duplicate domain names in domain_config.py: {ext_duplicates}")
        REPORT["duplicate_domains"] += list(ext_duplicates)
    # 6. Output JSON report
    if json_output_path:
        with open(json_output_path, 'w') as f:
            json.dump(REPORT, f, indent=2)
        logger.info(f"[INFO] JSON report written to {json_output_path}")
    return REPORT
