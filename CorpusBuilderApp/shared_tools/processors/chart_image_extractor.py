...
class ChartImageExtractor:
    ...
    def __init__(self, config: Optional[Dict] = None, project_config: Optional[Dict] = None):
        """Initialize chart image extractor
        ...
        if project_config:
            if project_config.get('processors') is not None and 'specialized' in project_config.get('processors', {}) and 'charts' in project_config.get('processors.specialized', {}):
                ...
                self.config = project_config.get('chart_image_extractor')
            else:
    ...
    def extract(self, file_path: str) -> Dict[str, Any]:
        """Extract chart images from file
        ...
        if self.config['formats']['png']:
            png_charts = self._extract_png(file_path)
            results['charts'].extend(png_charts)
        
        # Extract JPG charts
        if self.config['formats']['jpg']:
            jpg_charts = self._extract_jpg(file_path)
            results['charts'].extend(jpg_charts)
        
        # Extract SVG charts
        if self.config['formats']['svg']:
            svg_charts = self._extract_svg(file_path)
            results['charts'].extend(svg_charts)
        
        # Filter charts by size
        results['charts'] = [
            c for c in results['charts']
            if self.config['min_image_size'] <= c['size'] <= self.config['max_image_size']
        ]
        
        # Filter by quality
        results['charts'] = [
            c for c in results['charts']
            if c['quality_score'] >= self.config['quality_threshold']
        ]
        
        results['count'] = len(results['charts'])
        return results
    
    def _extract_png(self, file_path: str) -> List[Dict[str, Any]]:
        """Extract PNG charts"""
        # Implementation details...
        return []
    
    def _extract_jpg(self, file_path: str) -> List[Dict[str, Any]]:
        """Extract JPG charts"""
        # Implementation details...
        return []
    
    def _extract_svg(self, file_path: str) -> List[Dict[str, Any]]:
        """Extract SVG charts"""
        # Implementation details...
        return []
...
def run_with_project_config(project: 'ProjectConfig', verbose: bool = False):
    """Run chart image extraction with project configuration
    ...
    Returns:
        dict: Extraction results
    """
    extractor = ChartImageExtractor(
        project_config=project.get_processor_config('chart_extraction')
    )
...
def main():
    ...
    # Save results
    output_file = Path(args.corpus_dir) / 'chart_extraction.json'
