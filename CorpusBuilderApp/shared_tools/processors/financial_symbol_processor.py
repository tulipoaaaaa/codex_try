...
class FinancialSymbolProcessor:
    ...
    def __init__(self, config: Optional[Dict] = None, project_config: Optional[Dict] = None):
        """Initialize financial symbol processor
        ...
        if project_config:
            if project_config.get('processors') is not None and 'specialized' in project_config.get('processors', {}) and 'symbols' in project_config.get('processors.specialized', {}):
                # New master config structure
                self.config = project_config.get('processors.specialized.symbols')
            elif project_config.get('financial_symbol_processor') is not None:
                # Legacy project config structure
                self.config = project_config.get('financial_symbol_processor')
            else:
...
def run_with_project_config(project: 'ProjectConfig', verbose: bool = False):
    """Run financial symbol processor with project configuration
    ...
    """
    processor = FinancialSymbolProcessor(
        project_config=project.get_processor_config('financial_symbol_processor')
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Process financial symbols in corpus')
    ...
    if args.project_config:
        ...
        processor = FinancialSymbolProcessor()
        results = processor.process_directory(args.corpus_dir)
    
    # Save results
    output_file = Path(args.corpus_dir) / 'financial_symbol_processing.json'
