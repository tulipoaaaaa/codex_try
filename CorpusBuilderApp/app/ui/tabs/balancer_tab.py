...
class BalancerTab(QWidget):
    ...
    def setup_ui(self):
        ...
        current_layout.addWidget(SectionHeader("Current Corpus Distribution"))
        ...
        options_layout.addWidget(SectionHeader("Balancing Options"))
        ...
        controls_layout.addWidget(SectionHeader("Control Panel"))

        # Primary controls
        primary_controls = QHBoxLayout()
        self.refresh_btn = QPushButton("Refresh Corpus Stats")
        refresh_icon = icon_manager.get_icon_path(
            "Loading and processing indicator", by="Function"
        )
        if refresh_icon:
        ...
        reanalyze_icon = icon_manager.get_icon_path(
            "Success status and completion indicator", by="Function"
        )
        if reanalyze_icon:
        ...
        analyze_icon = icon_manager.get_icon_path(
            "Data analytics and visualization", by="Function"
        )
        if analyze_icon:
        ...
        self.collect_missing_btn.setToolTip(
            "Trigger collectors for missing/underrepresented domains based on corpus analysis."
        )
        ...
        balance_icon = icon_manager.get_icon_path(
            "Load balancing and data distribution", by="Function"
        )
        if balance_icon:
        ...
        export_icon = icon_manager.get_icon_path(
            "Main dashboard and analytics view", by="Function"
        )
        if export_icon:
        ...
        status_layout.addWidget(SectionHeader("Balancing Status"))
    ...
    def toggle_auto_analysis(self, enabled: bool):
        ...
        if enabled:
            self.analysis_timer.start(
                self.analysis_interval_minutes * 60 * 1000
            ...
            self.show_notification(
                "Auto-Analysis Disabled", "Periodic analysis has been turned off"
            )
    ...
    def reanalyze_corpus(self):
        ...
        if changes_detected:
            self.show_notification(
                "Corpus Changes Detected",
                "Significant changes found. Consider rebalancing.",
                "warning",
            )
            self.analyze_corpus_balance()
        else:
            self.show_notification(
                "Analysis Complete", "No significant changes detected."
            )
    ...
    def calculate_balance_score(self) -> float:
        ...
        if num_domains > 0:
            balance_score = max(
                0, 100 - (total_deviation / num_domains * 100 / max_possible_deviation)
            )
        else:
            balance_score = 0
        ...
        if imbalances:
            ...
            if self.enable_notifications.isChecked():
                self.show_notification(
                    ...
                    f"Found {len(imbalances)} domains requiring attention",
                    "warning",
                )
        else:
            ...
            if self.enable_notifications.isChecked():
                self.show_notification(
                    "Balance Analysis Complete", "Corpus is well balanced!"
                )
    ...
    def export_balance_report(self):
        ...
        if filename:
            try:
                ...
                self.show_notification(
                    "Report Exported", f"Balance report saved to {filename}"
                )
    ...
    def balance_corpus(self):
        ...
        if self.enable_notifications.isChecked():
            self.show_notification(
                "Corpus Balancing Started", "Beginning automatic corpus balancing..."
            )
    ...
    def stop_balancing(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            if self.enable_notifications.isChecked():
                self.show_notification(
                    "Balancing Stopped",
                    "Corpus balancing was stopped by user",
                    "warning",
                )
    ...
    def collect_for_missing_domains(self):
        ...
        QMessageBox.information(
            self,
            "Collect for Missing Domains",
            "Triggered collection for missing/underrepresented domains. Check logs for details.",
        )
