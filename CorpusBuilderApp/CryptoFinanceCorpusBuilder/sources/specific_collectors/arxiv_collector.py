...
class ArxivCollector(ApiCollector):
    ...
    def _download_papers(self, papers):
        ...
        for paper in papers:
            ...
            subfolder = category if category else 'general'
            
            # Download the PDF
            filepath = self.download_file(pdf_link, filename, subfolder)
            
            if filepath:
                # Add filepath to paper metadata
                paper['filepath'] = str(filepath)
                downloaded.append(paper)
        
        self.logger.info(f"Downloaded {len(downloaded)} papers")
        return downloaded
