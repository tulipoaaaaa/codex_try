...
class UpdatedISDACollector(WebCollector):
    """Updated collector for ISDA documentation"""
    
    def __init__(self, output_dir, delay_range=(3, 7)):
        super().__init__(output_dir, base_url='https://www.isda.org/category/documentation/', delay_range=delay_range)
        self.logger.name = "ISDACollector"  # Set logger name for consistent logs
        self.session = requests.Session()
        self.session.headers.update({
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"
        })
    
    def find_documentation_sources(self):
        """Discover ISDA documentation sources by crawling navigation and search results"""
        potential_urls = [
        ...
        for url in potential_urls:
            ...
            try:
                ...
                if not url_filename:
                    url_filename = "index"
                html_path = self.output_dir / f"isda_{url_filename}.html"
                ...
                if pdf_links:
                    ...
                    if doc_sections:
                        self.logger.info(f"Found {len(doc_sections)} document sections on {url}")
                        documentation_pages.append({
                            "url": url,
        ...
        try:
            ...
            search_path = self.output_dir / "isda_search_results.html"
        ...
        results_path = self.output_dir / "isda_documentation_sources.json"
    ...
    def collect(self, max_sources=5):
        """Collect documentation from discovered sources and download all PDFs"""
        sources = self.find_documentation_sources()
        self.logger.info(f"Collecting ISDA documentation from {len(sources)} sources")
        documents = []
        for i, source in enumerate(sources[:max_sources]):
            url = source.get("url")
            if not url:
                continue
            self.logger.info(f"Processing source {i+1}/{min(max_sources, len(sources))}: {url}")
            try:
                response = self.session.get(url)
