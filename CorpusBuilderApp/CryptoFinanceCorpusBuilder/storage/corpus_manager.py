...
class CorpusManager:
    ...
    def __init__(self, config, environment=None):
        """
        Initialize CorpusManager with ProjectConfig.
        ...
        if isinstance(config, str):
            config = ProjectConfig(config, environment=environment)
        
        self.config = config
        self.base_dir = config.corpus_dir
        self.base_dir.mkdir(parents=True, exist_ok=True)
        
        # Get domain configuration from ProjectConfig
        self.domain_config = config.domain_configs
